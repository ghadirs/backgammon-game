...
const BackgammonBoard: React.FC<Props> = ({
      ...
      for (let i = 0; i < 24; i++) {
        ...
        for (let j = 0; j < absCount; j++) {
          ...
          const spacing = Math.min(
            CHECKER_R * 2 + 2,
            (maxStackHeight - CHECKER_R) / Math.max(1, absCount - 1),
          );
  ...
  const handleCanvasClick = (e: React.MouseEvent) => {
    ...
    const inRightQuad =
      x > SIDEBAR_WIDTH + QUADRANT_WIDTH + BAR_WIDTH &&
    ...
    if (isTop) {
      if (inLeftQuad) {
        ...
        const offset = Math.floor(
          (x - (SIDEBAR_WIDTH + QUADRANT_WIDTH + BAR_WIDTH)) / POINT_W,
        );
        pointIdx = 18 + offset;
      }
    } else {
