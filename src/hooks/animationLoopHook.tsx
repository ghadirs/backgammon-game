...
export const useAnimationLoop = (
    ...
    useEffect(() => {
        ...
        const animate = (time: number) => {
            ...
            // --- DRAW SIDE TRAY (BEAR OFF) CHECKERS ---
            drawOffCheckers(boardRef.current.whiteOff, 1, ctx);
            drawOffCheckers(boardRef.current.blackOff, -1, ctx);
            ...
            drawBarCheckers(boardRef.whiteBar, 1, ctx);
            drawBarCheckers(boardRef.blackBar, -1, ctx)
            ...
            if (animatingChecker) {
                ...
                if (progress >= 1) {
                    // 1. Ref is already synced from executeAutoMove, but let's be safe
                    boardRef.current = animatingChecker.newBoardState;

                    // 2. Capture the state to pass to the parent
                    const finalPoints = animatingChecker.newPoints;

                    // 3. Clear animation state
                    setAnimatingChecker(null);

                    // 4. Notify parent - use a microtask to avoid React state batching issues
                    queueMicrotask(() => {
                        if (onMoveExecuted) {
                            onMoveExecuted(finalPoints);
                        }
                    });
                }
            }
