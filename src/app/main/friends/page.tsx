...
export default function PlayerRequestRow({
  ...
  return (
    ...
    <main className="w-full px-6 flex items-start justify-center mt-6">
      <div className="flex flex-col w-3/4 gap-2 h-[75vh] overflow-y-scroll items-center justify-between rounded-xl bg-[#09152A]/35 border-b border-[#1A3150] px-4 py-2 backdrop-blur-sm">
        {friends.map((e, idx) => (
          <div
            key={idx}
          ...
          >
            ...
            <button className="rounded-lg border border-[#2A4A75] bg-[#162A45]/50 px-4 py-2 text-xs font-semibold text-white transition-all hover:bg-[#0098EA] hover:border-[#0098EA] active:scale-95 whitespace-nowrap">
              Request Game
            </button>
          </div>
        ))}
